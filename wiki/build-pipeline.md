...
## The precision hook writes to these files, and it had two bugs

`~/.claude/scripts/precision_rules.py` runs on Stop and rewrites every vault `.md`
and `.html` touched since the last run. It is not part of `build.js`, it edits the
**sources** in `publish/analyses/` directly, silently, before you ever see a diff.
Both bugs below shipped to production on 2026-08-26 and were caught only by reading
the commit diff after pushing.

### Bug 1: it rewrote JavaScript

Rule R2 strips the space before `:`, `?` and `!`. Applied inside an inline
`<script>`, that hits every ternary:

```js
var trail = tN ? tSum / tN : null;      →   var trail = tN? tSum / tN: null;
set("k", a.since === 1 ? " day" : " days");  →   ...=== 1? " day": " days");
```

Twenty-five lines in `spy-747-level.html`. Still valid JavaScript, confirmed with
`node --check`, so nothing broke and nothing warned. A scan of every article
against `1288445` found this was the only file with a rewritten script block.

**Fixed:** `<script>` and `<style>` blocks are now emitted verbatim, tracked with
the same state machine the hook already used for Markdown fences.

### Bug 2: it turned date ranges into enumerations

Rule R1 rewrites a spaced dash as a comma. In a table of periods that inverts the
meaning:

| Was | Became |
|---|---|
| `<td>1999 – 2013</td>` | `<td>1999, 2013</td>` |
| `<td>-56.5% (2007 – 2009)</td>` | `<td>-56.5% (2007, 2009)</td>` |

Five cells in the `spy-747-level` levels table, live in production. "Active Era:
1999 to 2013" had become "1999 and 2013". The unspaced form was always handled
correctly (`2020–2026` → `2020-2026`); only the spaced one broke.

**Fixed:** a digit on both sides of the dash means a range, and takes a hyphen.
Prose keeps the comma, headings keep the colon. This matches the rule stated in
[1288445](https://github.com/Marcvrick/trading852-v2/commit/1288445): VOIX-Marc
allows an en dash only in a numeric range and writes those ranges with a hyphen.

### Both restored

`spy-747-level.html` has its original script block back, byte-for-byte from
`1288445`, and the five cells rewritten as `1999-2013` rather than as en dashes,
so the fixed hook is idempotent on them. `precision_rules.py --check` now reports
nothing on that file. Eight cases were added to the hook's selftest.

### What to do about it

- **Read the diff before pushing, not after.** Both bugs were invisible in the
  working tree and obvious in `git diff`. The hook edits files you did not edit.
- Its docstring claimed fenced blocks, inline code and frontmatter "covers every
  real case in this vault". It did not: this repo's `.html` carries inline JS and
  data tables. Treat that claim as a hypothesis, not a guarantee.
- The hook only touches files it sees change. Anything it would rewrite is latent
  until the next edit of that file, so a clean article today is not proof.
...
The small dark tab that slides down under the header on the homepage, announcing the single
freshest genuine update. Generated by `generateUpdateBannerHTML()` in [build.js](../build.js),
injected where [publish/index.html](../publish/index.html) carries the `{{UPDATE_BANNER}}` token.
Markup: `.update-banner-wrap` (fixed, full width, `pointer-events: none`, centring flex) wrapping
`.update-banner` (the tab itself, `translateY(-100%)` until a script adds `.is-shown`). Styles live
in [publish/styles/index.css](../publish/styles/index.css).

**No separate data file.** The banner reads the same `isRecentUpdate()` signal as the "Updated" tag
in the analyses list, so the two can never disagree: `modDate` at least `UPDATE_MIN_GAP_DAYS` (7)
after `pubDate`, and no more than `UPDATE_FRESH_DAYS` (30) old. Of the articles that pass, the one
with the newest `modDate` wins. Bumping `modDate` in an article's CONFIG, already required by the
update workflow, is the whole manual step; the next `node build.js` picks it up.
...
**Ported out (2026-08-21).** This component now has a second consumer: the FinMC_3 day trading
scanner reuses its geometry, its 0.5 s `cubic-bezier(0.2, 0.8, 0.2, 1)` glissade, and its
dismiss-by-key pattern for the order-block breakout alert, with `sessionStorage` in place of
`localStorage` and the breakout set in place of `slug-modDate`
(`day_trading_system/dashboard.py:_render_ob_banner`, documented in
[Day Trading App/wiki/order-blocks.md](../../Day%20Trading%20App/wiki/order-blocks.md)). It is a
copy, not a shared module, the two live in different stacks. Worth knowing before restyling the
tab here: the scanner will not follow on its own.
