...
- **Mobile menu, follow-up the same day.** Dany opened the panel on `/scorecard` and could not tell where the menu ended and the page began. Cause: the panel was `background: var(--dp-c-black)` over a page that is also black, separated by a white hairline at 12% opacity, which is invisible on black. The motion pass had made the panel arrive properly and left it indistinguishable once arrived. Fixed by giving the panel a faint blue wash (`linear-gradient(180deg, rgba(71,96,255,0.10), rgba(71,96,255,0.03))` layered over the black) and turning its bottom edge into a blue line plus a glow that fades onto the page (`border-bottom: 1px solid rgba(71,96,255,0.38)`, `box-shadow: 0 1rem 1.5rem -0.75rem rgba(71,96,255,0.30)`). Blue reuses the `71,96,255` of the homepage radials, so it reads as the same system on both the blue-lit home and the black scorecard hero.
