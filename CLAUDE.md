...
> **`DRAFT/` is empty as of 2026-07-15.** Its six drafts were deleted deliberately: the work had gone stale and the tickers left coverage. The folder is a workflow location, not a store, so recreate it on the next draft. Do not restore the old files; they stay in git history if ever needed. `DRAFT/` and `publish/drafts/` are not interchangeable, see the two states below.
...
> The authoritative checklist is [wiki/editorial.md](wiki/editorial.md) "Pre-publish checklist". This one is a card-and-CONFIG subset and the two have diverged before. When they disagree, editorial.md wins.

- [ ] Knowledge layer checked and written back in the same commit ([wiki/knowledge/](wiki/knowledge/index.md))
